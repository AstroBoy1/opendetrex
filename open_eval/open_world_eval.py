...
class OWEvaluator:
        ...
        for class_label_ind, class_label in enumerate(self.voc_gt.CLASS_NAMES):
            ...
            self.rec, self.prec, self.AP[class_label_ind, ovthresh_ind], self.unk_det_as_known, \
                self.num_unk, self.tp_plus_fp_closed_set, self.fp_open_set = voc_eval(lines_by_class, \
                                                                                      ...
                                                                                      known_classes=self.known_classes)  # [-1]
    ...
    def summarize(self, fmt='{:.06f}'):
        ...
        total_num_unk_det_as_known = {iou: np.sum(x) for iou, x in
                                      self.unk_det_as_knowns.items()}  # torch.sum(self.unk_det_as_knowns[:, o50]) #[np.sum(x) for x in self.unk_det_as_knowns[:, o50]]
        ...
        print("Current class AP50: " + str(
            self.AP[:, o50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls].mean()))
        print("Current class Precisions50: " + str(
            np.mean(self.precs[50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls])))
        print("Current class Recall50: " + str(
            np.mean(self.recs[50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls])))
        ...
        Res = {
            ...
            "CK_AP50": float(
                self.AP[:, o50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls].mean().detach().cpu()),
